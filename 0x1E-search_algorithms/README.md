...
- [skip list](https://youtu.be/ol-FaNLXlR0?feature=shared)
Learning Objectives
...
#advanced
**Tip**
- arr[low] <= key <= arr[high]: This condition ensures that the key being searched for lies within the range defined by the current low and high indices. It checks if the key is greater than or equal to the value at index low and less than or equal to the value at index high. This condition is crucial for maintaining the validity of the search space and preventing the algorithm from accessing indices outside the array bounds, which could lead to runtime errors
